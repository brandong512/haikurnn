...
class TrainingLine:
    def __init__(self, name, previous_line, lstm, n_tokens):
        ...
        self.syllable_dense = Dense(
            lstm.units, activation="relu", name="syllable_dense_%s" % name
        )
        ...
        if previous_line:
            initial_state = [
                Add(name="add_h_%s" % name)(
                    [previous_line.lstm_h, self.syllable_dense_output]
                ),
                Add(name="add_c_%s" % name)(
                    [previous_line.lstm_c, self.syllable_dense_output]
                ),
            ]
        else:
            initial_state = [self.syllable_dense_output, self.syllable_dense_output]

        self.lstm_out, self.lstm_h, self.lstm_c = lstm(
            self.char_input, initial_state=initial_state
        )

        self.output_dense = Dense(
            n_tokens, activation="softmax", name="output_%s" % name
        )
...
class GeneratorLine:
    def __init__(self, name, training_line, lstm, n_tokens):
        ...
        self.lstm_out, self.lstm_h, self.lstm_c = self.lstm(
            self.char_input, initial_state=initial_state
        )
...
class Generator:
    def __init__(self, lstm, lines, tf_session, tokenizer, n_tokens, max_line_length):
        ...
        self.lstm = LSTM(
            lstm.units, return_state=True, return_sequences=True, name="generator_lstm"
        )
        self.lines = [
            GeneratorLine("generator_line_%s" % i, lines[i], self.lstm, self.n_tokens)
            for i in range(3)
        ]
        self.lstm.set_weights(lstm.get_weights())

    def generate_haiku(self, syllables=[5, 7, 5], temperature=0.1, first_char=None):
        output = []
        h = None
        c = None
        ...
        for i in range(3):
                ...
                char, h, c = self.tf_session.run(
                    ...
                    feed_dict={
                        line.char_input: [
                            [
                                to_categorical(
                                    line_output[-1], num_classes=self.n_tokens
                                )
                            ]
                        ],
                        line.h_input: h,
            ...
            cleaned_text = (
                self.tokenizer.sequences_to_texts([line_output])[0]
                .strip()[1:]
                .replace("   ", "\n")
                .replace(" ", "")
                .replace("\n", " ")
